function getData() {
  function getStartDate() {
    return new Date(
      document
        .querySelector(
          `#sub_right > div:nth-child(3) > div > p:nth-child(1) > font:nth-child(2) > sup > span`
        )
        .innerText.replace(/[\(년월일]/g, "")
        .replace(/\ /g, ".")
    );
  }
  ...
  function getFoodList() {
    const nodelist = document.querySelectorAll(
      "#sub_right > div:nth-child(4) > div > table > tbody > tr:nth-child(2) > td"
    );
